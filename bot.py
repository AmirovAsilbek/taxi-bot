...
ADMIN_ID = 8726943857  # O'zingizning Telegram ID raqamingizni yozing
...
def add_driver_to_db(driver_id: int):
    conn = sqlite3.connect("taxi_bot.db")
    cursor = conn.cursor()
    cursor.execute("INSERT OR IGNORE INTO drivers (driver_id) VALUES (?)", (driver_id,))
    conn.commit()
    conn.close()
...
async def add_driver_callback(callback: types.CallbackQuery):
    ...
    if new_driver_id not in drivers:
        ...
        try:
            ...
            print(f"Haydovchiga bildirishnoma yuborishda xatolik: {e}")
    else:
...
async def send_order_to_available_driver(client_id, skip_drivers=None):
    ...
    if target_driver:
        ...
        try:
            ...
            print(f"Haydovchiga yuborishda xatolik: {e}")
            return False
    else:
        return False
...
async def process_location(message: types.Message, state: FSMContext):
    ...
    if sent:
        ...
        await message.answer("⚠️ Hozirda barcha haydovchilar band. Birozdan so'ng qayta urinib ko'ring.", reply_markup=main_keyboard)
...
async def reject_order(callback: types.CallbackQuery):
    ...
    if not sent:
        try:
            ...
            print(f"Mijozga ogohlantirish yuborishda xatolik: {e}")
